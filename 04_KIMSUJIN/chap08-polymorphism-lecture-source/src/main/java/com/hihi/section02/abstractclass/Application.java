...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        * 추상클래스는 클래스 선언부에 abstract키워드를 명시해야 한다.
        * 추상클래스로는 인스턴스를 생성할 수 없다.
        * 추상클래스를 사용하려면 추상클래스를 상속받는 하위클래스를
        * 이용해서 인스턴스를 생성해야 한다.
        ...
        * ->  자 바에서는 extends로 상속받을 시 하나의 부모 클래스만 가질 수 있다.(단일상속)
        *
        * 추상메소드란?
        * 메소드의 선언부만 있고 구현부가 없는 메소드를 추상메소드라고 한다.
        * 추상메소드의 경우 반드시 abstract 키워드를 메소드 헤드에 작성해야한다.
        * ex) public abstract void method();            => {}안쓰고 그냥 종결해버림
        *  */
...
//        Product product = new Product();  //'Product' is abstract; cannot be instantiated
        ...
        /*다형성 적용해서 추상클래스를 레퍼런스 타입으로 사용할 수 있다.*/
