...
public class Application2 {
    ...
    public static void main(String[] args) {
        ...
        * new 연산자는 heap영역에 공간을 할당하고 발생한 주소값을 반환
        * 발생한 주소가 레퍼런스 변수(참조형변수)에 저장되고 이것을 참조하여 사용하기 때문에
        * 참조 자료형이라 함.
        * */
        iarr = new int[5]; // int값을 5개 관리하기위한 공간을 만듬
        darr = new double[10];

        /*
        * heap영역은 이름으로 접근하지 않고 주소값으로 접근하는 영역
        * sstack에 저장된 주소로 heap에 할당 된 배열을 찾아갈 수 있음
        * */
        ...
        * String 클래스는 메소드로 제공하여 length()로 사용하는 점 유의하기
        * */
        System.out.println("iarr = " + iarr.length);
        System.out.println("darr = " + darr.length);

        /*
        * 한 번 지정한 배열의 길이는 변경 XX
        * 새로운 배열을 생성하여 그 주소값을 레퍼런스 변수에 담음
        * */
        iarr = new int[100];
        ...
        /*
        * 레퍼런스 변수가 아무런 주소를 참조하지 않는 상태를 null이라 함
        * 100개 짜리의 배열이 더 이상 참조 되지 않으므로 GC(가비지 컬렉터)에 의해 제거 됨
        * */
        iarr = null;

        /*
        * null이라는 특수한 값을 참조하고 있을 경우 참조 연산자(.) 사용기
