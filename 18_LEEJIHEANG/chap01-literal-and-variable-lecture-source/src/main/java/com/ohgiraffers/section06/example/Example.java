...
public class Example {
    /*
        ...
        요구사항:
        상수(Constant): 부가세율(T을 0.1 (10%)로 하는 상수를 선언하세요.
        변수(Variable):
        ...
        int price = 12000;
        int quantity = 3;
        int Pretax = price * qunatity;
        double Tax = Pretax * TAX_RATE
        double After-tax = Pre-tax + Tax
    ...
    static void main() {
        ...
        // 수량(quantity)
        int quantity = 3;
        // 세전 금액(Pre_tax)는 가격 * 수량
        int preTax = price * quantity;
        // 부가세(Tax)는 세전 금액 * 부가세율
        double tax = preTax * taxRate;
        // 세후 금액는 세전 금액 + 부가세
        double afterTax = preTax + tax;
        // 최종 결제 금액은 세후 금액의 정수값
        int finalTax = (int) afterTax;

        System.out.println("세전금액: " + preTax + "원");
        System.out.println("부가세: " + tax + "원");
        System.out.println("세후 금액: " + afterTax + "원");
        System.out.println("최종 결제 금액: " + finalTax + "원");
    }
