...
public class Application2 {
    ...
    public static void main(String[] args) {
        String[] str={"홍길동","이순신","유관순","윤봉길","장영실","임꺽정"
                ,"장보고","이태백","김정희", "대조영","김유신","이사부"};
        int num=0;
        String[][] d1= new String[3][2];
        for(int i=0; i<d1.length; i++){
            d1[i]=Arrays.copyOfRange(str,num,num+2);
            num+=2;
        }

        String[][] d2=new String[3][2];
        for(int i=0; i<d1.length; i++){
            d2[i]=Arrays.copyOfRange(str,num,num+2);
            num+=2;
        }
