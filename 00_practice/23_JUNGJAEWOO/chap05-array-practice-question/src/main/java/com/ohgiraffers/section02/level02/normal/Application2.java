...
public class Application2 {
    ...
    public static void main(String[] args) {
         ...
         */
        String[] student;
        student = new String[]{"홍길동", "이순신", "유관순", "윤봉길", "장영실", "임꺽정", "장보고", "이태백", "김정희", "대조영", "김유신", "이사부"};
        String[][] team1 = new String[3][2];
        String[][] team2 = new String[3][2];
        int row = 0;
        for (int i = 0; i < student.length; i++) {
            if(i < 2) {
                team1[row][i % 2] = student[i];
            } else if(i < 4) {
                team1[row+1][i % 2] = student[i];
            } else if(i < 6){
                team1[row+2][i%2] = student[i];
            } else if(i < 8){
                team2[row][i%2] = student[i];
            } else if(i < 10){
                team2[row+1][i%2] = student[i];
            } else {
                team2[row+2][i%2] = student[i];
            }
        }
