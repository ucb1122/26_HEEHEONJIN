...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        Animal animal1 = new Rabbit(); // Rabbit이 Animal로 업캐스팅
        Animal animal2 = new Tiger();

        /* Animal은 Animal 타입이지 Tiger나 Rabbit이 아니다.*/
//        Rabbit rabbit1 = new Animal(); // 서로 타입이 안맞음
//        Tiger tiger1 = new Animal(); // 여기도 타입이 안맞음
        ...
        * ==> 타입 형변환시 실제 인스턴스와 타입이 일치하지 않는 경우 ClassNotFoundException이 발생할 수 있다.
        * */
        System.out.println("==== 클래스타입 형변환 확인 ====");
        ((Rabbit)animal1).jump();
        ((Tiger)animal2).bite();

//        ((Tiger)animal1).bite(); // ClassNotFoundException 발생
        System.out.println();
        /*
        * instanceof 연산자 이용
        ...
        * */
        ...
        Animal animal3= (Animal) new Rabbit(); // 명시적 형변환 (up-casting)
        Animal animal4 = new Rabbit(); // 묵시적 형변환 (up-casting)

        Rabbit rabbit1 = (Rabbit) animal3; // 명시적 형변환 (down-casting)
        // 다운캐스팅은 묵시적 형변환 X
