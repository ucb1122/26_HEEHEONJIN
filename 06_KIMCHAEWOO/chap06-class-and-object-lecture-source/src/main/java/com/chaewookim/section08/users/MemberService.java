package com.chaewookim.section08.users;

import java.util.Arrays;

public class MemberService {

    // 회원가입
    public void signUpMembers() {

        Member[] members = new Member[5]; // null, null, null, null, null -> 해당 객체가 들어갈 전체 배열만 만들어진 것이고 안에 들어간 게 없기 때문에 값이 null로 되는 것

        members[0] = new Member(1, "user01", "pass01", "홍길동", 20, '남');
        members[1] = new Member(2, "user02", "pass02", "유관순", 16, '여');
        members[2] = new Member(3, "user03", "pass03", "이순신", 40, '남');
        members[3] = new Member(4, "user04", "pass04", "신사임당", 30, '여');
        members[4] = new Member(5, "user05", "pass05", "윤봉길", 22, '남');

        MemberRegister memberRegister = new MemberRegister();
        memberRegister.regist(members);

    }

    // 회원 조회
    public void showAllMembers() {
        MemberFinder memberFinder = new MemberFinder();
        System.out.println(Arrays.toString(memberFinder.findAllMembers()));
    }

}
